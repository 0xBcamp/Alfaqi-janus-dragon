...
contract BlockMedSecure {
    ...
    // mapping(address => MedicalReport[]) public medicalReports; // Replaced with FileStorage contract
    ...
    function addDoctor( address walletAddress, string memory name, uint256 timeExperience, string memory specialty ) external onlyAdmin {
    ...
    function writeMedicalReport( address patientAddress, uint256 reportNumber, string memory reportDate, string memory ipfsHash) external onlyVerifiedDoctor {
        // medicalReports[patientAddress].push(MedicalReport({ // Replaced with FileStorage contract
        ...
        // doctors[msg.sender].reportsHistory.push(ipfsHash); // Replaced with FileStorage contract
     ...
     */
    // function getMedicalReports(address patientAddress) external view returns (MedicalReport[] memory) { // Replaced with FileStorage contract
